...
def gen_frames():
    ...
    while True:
        ...
        for z_name, poly in POLYGON_ZONES.items():
            ...
            for app_type in ["light", "fan"]:
                mode = manual_overrides[z_name][app_type]
                if mode == "FORCE_ON":
                    update_appliance(z_name, app_type, True)
                elif mode == "FORCE_OFF":
                    update_appliance(z_name, app_type, False)
                else:
                    update_appliance(z_name, app_type, is_active)
...
<head>
    ...
    <style>
        ...
        .btn-alarm { background: linear-gradient(45deg, #ff4757, #ff6b81); color: white; width: 100%; padding: 14px; font-size: 14px; text-transform: uppercase; border-radius: 12px; font-weight: bold; cursor: pointer; border: none; }
...
<body>
    ...
    <div class="grid">
        ...
        <div style="display: flex; flex-direction: column; gap: 25px;">
            ...
            <div class="card">
                <h3>System Audio Alarm</h3>
                <button class="btn-alarm" onclick="playSirenSound(true)">🚨 Sound Alarm Siren</button>
                <button class="btn-silence" onclick="playSirenSound(false)">Silence Siren</button>
                
                <h4 style="margin-top:20px; color:#666; font-size: 11px; text-transform: uppercase;">Live Gesture Alerts</h4>
                <div id="alerts"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Audio synthesizer for Dashboard Alarm Siren
        let audioCtx = null;
        let sirenOsc = null;
