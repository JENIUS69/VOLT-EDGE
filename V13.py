...
def poll_environment_sensor():
    ...
    while True:
        try:
            r = requests.get(f"{ESP_IP}/sensor", timeout=1.5)
            if r.status_code == 200: 
                sensor_data = r.json()
        except Exception as e: 
            print(f"[SENSOR ERROR] Failed to fetch telemetry: {e}")
        time.sleep(3)

threading.Thread(target=poll_environment_sensor, daemon=True).start()

# --- 4. VISION PIPELINE & NETWORK DISPATCH ---
...
def async_send(url):
    try:
        ...
        print(f"[ESP32 OK] {url} -> HTTP {r.status_code}")
    except Exception as e:
        print(f"[ESP32 FAIL] Could not reach {url}: {e}")
...
<!DOCTYPE html>
<html>
<head><title>VoltEdge OS | Login</title>""" + COMMON_STYLES + """</head>
<body style="display:flex; justify-content:center; align-items:center; height:90vh;">
    <div class="card" style="width: 320px; text-align: center; padding: 40px;">
        ...
        <form method="POST">
            <input type="text" name="username" placeholder="Username" required style="width:100%; padding:12px; margin-bottom:12px; background:#070c18; border:1px solid var(--border); color:#fff; border-radius:8px; box-sizing:border-box;">
            <input type="password" name="password" placeholder="Password" required style="width:100%; padding:12px; margin-bottom:20px; background:#070c18; border:1px solid var(--border); color:#fff; border-radius:8px; box-sizing:border-box;">
...
<body>
    <div class="nav">
        <div class="nav-title"><h2 style="margin:0;">⚡ VoltEdge OS</h2><span class="badge">ADMIN CONSOLE</span></div>
    ...
    <div class="grid-2">
        <div>
            <div class="card"><h3 style="margin-top:0;">📹 Optics Stream</h3><img src="/video_feed" style="width:100%; border-radius:8px;"></div>
            <div class="card">
                ...
                <div class="zone-grid">
                    ...
                    <div class="zone-box">
                        ...
                        <div style="margin-top:8px;">
                            ...
                            <button class="btn btn-dark" onclick="setOverride('{{z}}', 'light', 'AUTO')">AUTO</button>
                        </div>
                        <div style="margin-top:6px;">
                            ...
                            <button class="btn btn-dark" onclick="setOverride('{{z}}', 'fan', 'AUTO')">AUTO</button>
                        </div>
                    </div>
                    {% endfor %}
                </div>
            </div>
            <div class="card"><h3 style="margin-top:0;">📈 Zone Occupancy Analytics</h3><canvas id="chart" height="110"></canvas></div>
        </div>
        <div>
            <div class="card">
            ...
            <div class="card"><h3 style="margin-top:0;">🛎️ Service & Incident Logs</h3><div id="service-logs" style="max-height:350px; overflow-y:auto;"></div></div>
        </div>
    </div>
    ...
    <script>
        ...
        async function setOverride(zone, appliance, mode) { await fetch('/api/override', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({zone, appliance, mode}) }); }
        async function setCutoff() { let val = parseFloat(document.getElementById('cutoff-in').value); await fetch('/api/temp_cutoff', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({temp_cutoff: val}) }); }
...
<!DOCTYPE html>
<html>
<head><title>VoltEdge OS | Assistant</title>""" + COMMON_STYLES + """</head>
<body>
    <div class="nav">
        <div class="nav-title"><h2 style="margin:0;">⚡ VoltEdge OS</h2><span class="badge">ASSISTANT</span></div>
        <a href="/logout" class="logout-btn">LOGOUT</a>
    </div>
    <div class="telemetry-bar">
        <div class="t-box"><div class="t-label">TEMP</div><div class="t-val"><span id="t-temp">--</span> °C</div></div>
        <div class="t-box"><div class="t-label">HUMIDITY</div><div class="t-val"><span id="t-hum">--</span> %</div></div>
        <div class="t-box"><div class="t-label">OCCUPANTS</div><div class="t-val" id="t-count">0</div></div>
        <div class="t-box"><div class="t-label">CALLS</div><div class="t-val" id="t-req-count">0</div></div>
    </div>
    <div class="grid-2">
        <div class="card"><h3 style="margin-top:0;">🛎️ Service Calls</h3><div id="request-queue"></div></div>
        <div class="card"><h3 style="margin-top:0;">📹 Stream</h3><img src="/video_feed" style="width:100%; border-radius:8px;"></div>
    </div>
    <script>
        async function acceptReq(reqId) { await fetch('/api/complete_request', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({id: reqId}) }); }
        setInterval(async () => {
            let r = await fetch('/api/stats');
            let d = await r.json();
            document.getElementById('t-temp').innerText = d.temp;
            ...
            document.getElementById('request-queue').innerHTML = d.requests.map(s => 
                `<div class="req-item"><div><strong>${s.item}</strong></div>${s.status === 'DONE' ? '<span class="badge">DONE</span>' : `<button class="btn btn-green" onclick="acceptReq(${s.id})">ACCEPT</button>`}</div>`
            ).join('');
        }, 1500);
    </script>
</body>
...
<!DOCTYPE html>
<html>
<head><title>VoltEdge OS | Guest</title>""" + COMMON_STYLES + """</head>
<body>
    <div class="nav">
        <div class="nav-title"><h2 style="margin:0;">⚡ VoltEdge OS</h2><span class="badge">GUEST</span></div>
        <a href="/logout" class="logout-btn">LOGOUT</a>
    </div>
    <div class="telemetry-bar">
        <div class="t-box"><div class="t-label">TEMP</div><div class="t-val"><span id="t-temp">--</span> °C</div></div>
    ...
    <div class="card">
        ...
        <button class="btn btn-cyan" onclick="sendReq('💧 WATER REQUEST')">Request Water</button>
        <button class="btn btn-blue" onclick="sendReq('☕ COFFEE REQUEST')">Request Coffee</button>
    </div>
    <script>
        async function sendReq(item) { await fetch('/api/service_request', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({item, zone: 'GUEST ROOM'}) }); }
        setInterval(async () => {
            let r = await fetch('/api/stats');
            let d = await r.json();
            document.getElementById('t-temp').innerText = d.temp;
...
@login_required
def dashboard():
    if current_user.role == 'admin': return render_template_string(ADMIN_DASHBOARD_TEMPLATE, user=current_user)
    elif current_user.role == 'assistant': return render_template_string(ASSISTANT_DASHBOARD_TEMPLATE, user=current_user)
    else: return render_template_string(GUEST_DASHBOARD_TEMPLATE, user=current_user)
...
def create_service_request():
    ...
    d = request.json
    time_str = datetime.now().strftime("%H:%M:%S")
    service_requests.insert(0, {"id": req_id_counter, "item": d.get("item", "ASSIST"), "time": time_str, "status": "PENDING", "zone": d.get("zone", "GUEST")})
    req_id_counter += 1
    return jsonify({"status": "success"})
...
def complete_service_request():
    if current_user.role not in ['admin', 'assistant']: return jsonify({"error": "Unauthorized"}), 403
